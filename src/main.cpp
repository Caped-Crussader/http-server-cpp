...
int main(void) {
  ...
  auto send_html = [](const HttpRequest& request) -> HttpResponse {
    ...
    content += "<html>\n<body>\n\n";
    content += "<h1>Hello, world, the Server is now running</h1>\n";
    content += "<p>Success</p>\n\n";
    content += "</body>\n</html>\n";
  ...
  try {
    ...
    std::cout << "Enter [quit] or press Enter to stop the server" << std::endl;
    std::string command;
    while (true) {
      std::getline(std::cin, command);
      if (command.empty() || command == "quit") {
        if (command.empty())
          std::cout << "Enter pressed. Stopping the web server.." << std::endl;
        else
